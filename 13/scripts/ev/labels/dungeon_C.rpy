label dungeon_C:
    menu:
        "让她躺在床上":
            jump dungeon_C.c_dungeon_1
        "固定她的头部和手臂":

            jump dungeon_C.c_dungeon_2

    jump dungeon.end

label dungeon_C.c_dungeon_1:

    scene void with tstmgr
    play music happy
    "... ... ... ..."



    scene c_dungeon_1_1 with dissolve

    c "难... 难以置信... ..."



    scene c_dungeon_1_2 with tstmgr

    c "你怎么敢把一个假鸡巴......放进我身体里?我从来没有... awwwwww... ..."



    player "但是你已经开始开始爱上它的感觉了，对吧?"



    c "它相比于你的鸡巴... 不值一提... ..."



    scene c_dungeon_1_3 with tstmgr

    player "Ohhhh... 宝贝~ 真庆幸听到了这些~"



    scene c_dungeon_1_4 with tstmgr

    player "所以现在... ..."



    scene c_dungeon_1_5 with tstmgr

    pause



    scene c_dungeon_1_6 with tstmgr

    pause



    scene c_dungeon_1_5 with tstmgr

    pause



    scene c_dungeon_1_6 with tstmgr

    pause



    scene c_dungeon_1_5 with tstmgr

    pause



    scene c_dungeon_1_6 with tstmgr

    pause



    scene c_dungeon_1_5 with tstmgr

    pause



    scene c_dungeon_1_7 with tstmgr

    player "妈的，我可以和你的胸部玩上一整天~"



    c "我们在一起时，你不能每次都玩我的胸部.难道你就没有别的花样了吗?"



    player "哦......我明白了，那我们就试试新东西吧~"



    scene c_dungeon_1_8 with tstmgr

    pause



    c "呃......你是要... ..."



    scene c_dungeon_1_9 with tstmgr

    $ flashlight()

    c "Ohhhhhhh!~~~~~"



    scene c_dungeon_1_8 with tstmgr

    pause
  


    scene c_dungeon_1_9 with tstmgr

    $ flashlight()

    c "你敢! 你怎么能这样拍打我的胸部!~~~"



    scene c_dungeon_1_8 with tstmgr

    pause



    scene c_dungeon_1_9 with tstmgr
    play sound spank
    $ flashlight()

    c "Yeeeeeeeeee~~~~~~"



    scene c_dungeon_1_8 with tstmgr

    pause



    scene c_dungeon_1_9 with tstmgr
    play sound spank
    $ flashlight()

    pause



    scene c_dungeon_1_8 with tstmgr

    pause



    scene c_dungeon_1_9 with tstmgr
    play sound spank
    $ flashlight()

    c "我... 我撑不住了!~~~~"



    scene c_dungeon_1_10 with tstmgr
    play sound cumshot
    $ flashlight()

    c "Ohhhhhhhhhhhhhhh!~~~~~~~~"



    scene c_dungeon_1_11 with tstmgr

    player "那么~到底是什么让你射了?是假阳具还是胸上的打击?"



    c "我... 我不知道... ..."



    player "无论哪种方式，都证明你是个变态~"



    scene void with tstmgr

    "... ... ... .."
    stop music fadeout 1.0
    jump dungeon.end

label dungeon_C.c_dungeon_2:

    scene void with tstmgr
    play music happy
    "... ... ... ..."



    scene c_dungeon_2_1 with dissolve

    c "啊，该死......我已经开始后悔了... ..."



    c "这......太操蛋了... ..."



    player "来吧~会没事的~"



    c "我不认为这... ..."



    scene c_dungeon_2_2 with tstmgr
    play sound c_dungeon_2_2 loop
    pause



    c "啊啊啊!~去你的，我还没说完这句话呢!"



    player "但我就是不能再等了~"



    player "我怎么能拒绝你这美丽的大屁股呢?"



    c "Ah~~~ 操你妈的~~~~~~"



    player "但现在我可以用我想要的各种方式操你，而你甚至不能移动~"



    c "我憎恨你!!~~~~~~"


    stop sound
    scene c_dungeon_2_3 with tstmgr
    play sound c_dungeon_2_3 loop
    pause



    player "闭嘴吧.你现在是我的囚犯，你现在唯一能做的就是把你的屁股抬起来，迎接我的鸡巴!~"



    c "我... 我... ... Ohhhhhhh~~~~ awwwhhhh~~~~"



    player "你的小穴越来越湿了.我说的话让你更兴奋了吗?"



    player "你真他妈好色~"



    c "Ahhh~~ uhhhh~~ 我~~~ awwwwww 算了~~~~~"



    c "我只有在你面前才会变得饥渴...~~~~~"



    player "我的好女孩~"



    player "射了!~~~~~"


    stop sound
    scene c_dungeon_2_4 with tstmgr
    play sound cumshot
    $ flashlight()

    c "Ohhhhhhhhhhhhhhhhhhhhh!!!!!~~~~~~~"



    scene c_dungeon_2_5 with tstmgr

    player "啊~这真是太棒了... ..."



    c "你这个......混蛋... ..."



    player "你想用这个姿势再来一轮吗?~"



    c "... ... ... ..."



    c "都行啦~"
    stop music fadeout 1.0
    jump dungeon.end
# Decompiled by unrpyc: https://github.com/CensoredUsername/unrpyc
