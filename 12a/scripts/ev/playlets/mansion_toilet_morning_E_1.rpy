label mansion_toilet_morning_E_1:

    scene mansion_toilet_morning_e_1_1 with tstmgr

    e "Emmmm, [P]?"



    player "Eh,对不起,阿姨,门没锁所以......"



    scene mansion_toilet_morning_e_1_2 with tstmgr

    e "... ... ... ..."



    scene mansion_toilet_morning_e_1_3 with tstmgr

    e "反正我已经上完了~"



    player "*使劲吞咽*............"



    scene mansion_toilet_morning_e_1_4 with tstmgr

    e "Oh?"



    scene mansion_toilet_morning_e_1_5 with tstmgr

    e "你在看什么呢,坏小子?"



    player "你在给我看什么,坏阿姨?"



    e "Humph~"



    scene mansion_toilet_morning_e_1_6 with tstmgr

    e "我在给你看...你喜欢看的东西~"



    player "Ohhh...伊莉莎!"



    scene mansion_toilet_morning_e_1_7 with tstmgr

    d "*从另一个房间传来的大声询问*妈,你在跟谁说话?[P]在这里吗?"



    e "Awwww!~"



    scene mansion_toilet_morning_e_1_8 with tstmgr

    e "你...你最好现在就离开这个房间..."



    player "Oww......太可惜了......"



    scene void with tstmgr

    "... ... ... ..."


    $ add(E, E.love, 1)

    jump playlet_post
# Decompiled by unrpyc: https://github.com/CensoredUsername/unrpyc
