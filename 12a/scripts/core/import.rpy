...
init 1 python:
    ...
    def load_events():
        ...
        with renpy.file('events.json') as stream:
    ...
    def load_subplots():
        with renpy.file('subplots.json') as stream:
            store.SUBPLOTS = json.load(stream)


    def load_interactions():
        with renpy.file("interactions.json") as stream:
            data_loaded = json.load(stream)
            for arg_dict in data_loaded:
                Interaction(**arg_dict)

    def load_playlets():
        with renpy.file("playlets.json") as stream:
            data_loaded = json.load(stream)
            for arg_dict in data_loaded:
    ...
    def load_gallery():
        ...
        with renpy.file("gallery.json") as stream:
            data_loaded = json.load(stream)
            for arg_dict in data_loaded:
    ...
    def load_roomCGs():
        args = []
        with renpy.file("roomcgs.json") as stream:
            data_loaded = json.load(stream)
            for arg_dict in data_loaded:
                convert_enums(arg_dict)
                OneCG(**arg_dict)
