...
screen file_slots(title):
            ...
            fixed:
                ...
                button:
                    ...
                    if title == "存档":
                        ...
                        hovered SetLocalVariable("desc", True), MenuNotify("{color=#e8888a}右键点击{/color}删除存档.")
                ...
                text FileTime(slot, format=_("{#file_time}%Y-%m-%d %H:%M"), empty=_("空档")):
    ...
    if renpy.variant("touch") and title == "读档":
        if title == "读档":
            use game_menu_notify("{color=#e8888a}长按{/color}删除存档.", size=32)
    else:
