...
init -2 python:
    ...
    def add(owner, obj, value, chance=0, handle=True):
        ...
        if r != 0.0:
            ...
            Push(_("{}的{}没变").format(owner, obj))

        if obj.is_stage_locked:
            _stage_lock_notification(owner, obj)

    def _stage_lock_notification(owner, obj):
        if (obj is A.love and obj >= 30) or (obj is B.love and obj >= 35) or (obj is D.love and obj >= 35):
            Push(_("{}的{}已经满了,在她的房间找到她提升她的属性.").format(owner, obj))
        elif obj is F.love and obj >= 25 and not seen("A_love_5"):
            Push(_("在你改善与薇拉的关系后,瑞秋的更多故事将会被解开."))
        elif obj is F.love and obj >= 30 and not seen("B_train_inti_1"):
            Push(_("在你改善与森柠的关系后,瑞秋的更多故事将会被解开."))
        elif obj is F.love and obj >= 35 and not seen("G_train_sha_2"):
            Push(_("在你改善与乌诺的关系后,瑞秋的更多故事将会被解开."))
        elif obj is F.love and obj >= 40 and not seen ('ACG_duo_4'):
            Push(_("你应该先提高维拉、西奥和乌诺的后宫接受度."))
        else:
    ...
    def sub(owner, obj, value):
        r = obj.sub(value)
        Push(_("{}的{}增加了{}").format(owner, obj.name, value))


    def not_implemented_message():
        Push("对不起,这个玩法将在未来的版本中实现.")
    ...
    class Handle(object):
        ...
        def unchanged(cls, obj):
            Push(_("{}的{}没变").format(obj.owner.name, obj.name))
