init python:
    def get_findee_take_to_locs():
        ...
        if seen("found_pool") and get_findee_training_level("inti") >= 2 and findee.code != 'G':
            r.append(("地下泳池", "pool"))
        if seen("found_dungeon") and get_findee_training_level("sub") >= 2:
            r.append(("地牢", "dungeon"))
        if seen("found_outdoors") and get_findee_training_level("shame") >= 2 and findee.code not in ['A', 'B']:
